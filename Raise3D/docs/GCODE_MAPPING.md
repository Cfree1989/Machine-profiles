# Start / end G-code mapping

Required before treating any PrusaSlicer G-code as final.

Sources (ideaMaker 5.4.2.8790):

- Left-only: `LeftonlyExtruder.gcode`
- Right-only: `RightonlyExtruder.gcode`
- Dual / two-color: `MulticolorRaise3d.gcode`

Proposed output lives in `vendor/Raise3D.ini` printer start/end/tool-change G-code.

This mapping is **experimental**. Physical Stage 2–7 tests are still required.

## Start G-code

| ideaMaker (source) | PrusaSlicer equivalent | Action | Reason |
| --- | --- | --- | --- |
| `M99123 /RKIIyAfrg…` (line 1) | Same payload in start G-code; thumbnails disabled; `scripts/ensure_m99123_first.py` moves it to physical line 1 after export | Copied + post-export move | PrusaSlicer always prepends `; generated by …` (and used to emit PNG thumbnails). ideaMaker and [MackDan’s notes](https://forum.prusa3d.com/forum/prusaslicer/prusa-slicer-profile-for-raise3d-pro2-dual-head-printer/paged/2/) treat line 1 as the Hyper Speed handshake. Green checkmark is still a physical test. |
| `;Print Time:` / `;Material#N Used:` / per-layer `;PRINTING_TIME:` `;REMAINING_TIME:` | `remaining_times = 1` emits `M73 P… R…`. [Prusa M73](https://help.prusa3d.com/article/prusa-firmware-specific-g-code-commands_112173): `P` = percent, `R` = remaining **minutes**. [Post-processing](https://help.prusa3d.com/article/post-processing-scripts_283913) gets the temp G-code as the last argument and must rewrite it in place. We drop `M73` (RaiseTouch is not Prusa firmware) and write ideaMaker second counts on the two lines immediately before each `;LAYER:N` (not `;LAYER_CHANGE`). At `P0` / `;LAYER:0`, remaining uses the footer second total so M73's minute rounding does not show time already printed. PrusaSlicer skips Before layer change G-code on the first layer, so post-process synthesizes `;LAYER:0` plus that clock and the first `;LAYER_CHANGE` `;Z:` / `;HEIGHT:` immediately after `M1001`. `;Print Time:` is copied after `;Firmware:` and at EOF. If `M73` is missing, remaining time is interpolated from the layer index. **Do not put `HEIGHT` in Before layer change G-code** — PrusaSlicer treats it as a reserved keyword and warns that visualization and time estimation may break. Post-process copies the slicer's own `;HEIGHT:` (from `;LAYER_CHANGE`) to after `;LAYER:N` / `;Z:`. | Copied dialect | [ideaMaker 4.1.0](https://www.ideamaker.io/ideamaker-4-1-0-release-notes.html): `;PRINTING_TIME:` / `;REMAINING_TIME:` are **seconds already printed** and **seconds remaining**. Known-good ideaMaker files (and the same block in [Moonraker #759](https://github.com/Arksine/moonraker/issues/759)) place them immediately before `;LAYER:N`, then `;Z:` / `;HEIGHT:`. PrusaSlicer's `;LAYER_CHANGE` is a different marker; parking the clock there leaves `;Z:` / `;HEIGHT:` between the comments and `;LAYER:N`. [Raise3D](https://support.raise3d.com/General-Question/can-i-use-another-slicer-14-398.html): third-party slicers do not write `.data`. [ideaMaker export formats](https://support.raise3d.com/ideaMaker/1-3-2-exported-file-formats-15-1899.html): `.data` holds the **thumbnail** and slicing settings — not G-code PNG comments. File-picker 0s/0g/0$ / preview still need `.data`. |
| `;Sliced by ideaMaker …` | Omitted | Omitted | Lying about the slicer did not clear Hyper Speed warnings in [MackDan’s notes](https://forum.prusa3d.com/forum/prusaslicer/prusa-slicer-profile-for-raise3d-pro2-dual-head-printer/paged/2/). The “not sliced by ideaMaker” popup is expected. |
| `;Dimension: 305.000 305.000 605.000 0.400 0.400` | Same comment | Copied | Community: Raise3D Klipper reads this for nozzle-diameter check. Also matches this file. |
| `;Plate Shape: 0` / `;Origin Center: 0` / `;Extruder Offset #1: 0.000 0.000` | Same | Copied | Present in known-good header. |
| `;Filament Name #1: [Raise3D] PLA` | `{"[Raise3D] "}{filament_type[0]}` in custom G-code (`#2` uses `[1]`). PLA still emits `[Raise3D] PLA`. | Copied (escaped prefix) | RaiseTouch compares this to the name loaded in the slot. Bare `[Raise3D]` is a PrusaSlicer placeholder and blocks export. Wizard presets are named `PLA Raise3D` (and PETG/TPU/ASA/PA-CF/ABS-GF Raise3D). |
| `;Filament Diameter/Type/#` comments | Emitted from filament start G-code with placeholders | Copied (values parameterized) | Header metadata used by RaiseTouch. |
| `;Printer Type: RAISE3D Pro2 Plus - Hyper Speed` | Same | Copied | Confirmed in this file. |
| `;Firmware: Klipper` | Same | Copied | Confirmed in this file. |
| `;Bounding Box:…` | `ensure_m99123_first.py` writes object AABB after `;Firmware:`: XY from G0/G1 after `M1001`, Z from max `;Z:`. Not `{max_layer_z}` in start G-code. Rewrites a prior box when it is not valid ideaMaker order (`xmax < xmin` or `ymax < ymin`). | Copied dialect (post-export) | `{max_layer_z}` is not valid in start G-code ([placeholders](https://help.prusa3d.com/article/list-of-placeholders_205643)). ideaMaker writes `xmin xmax ymin ymax zmin zmax` of the print (not the full bed). |
| `M221 T0 S94.00` | PLA: `M221 T0 S94` (and T1) from filament_type PLA, even though `extrusion_multiplier` stays **1.00**. Other materials still use `S{extrusion_multiplier*100}`. `ensure_m99123_first.py` then scales first-layer print E ×0.90 and top-solid E ×1.06 | Copied dialect | ideaMaker writes per-feature FLOW into E and applies filament 94% with M221. Setting multiplier to 0.94 *and* M221 S94 would double-count. Compensation Test.gcode first-wall E matches 90% of nominal, not 84.6% — M221 supplies the 94%. End G-code still resets `M221 … S100`. |
| `M140 S60` / `M104 T0 S230` / `M109 T0 S230` / `T0` / `M190 S60` | Same commands with `{first_layer_bed_temperature[0]}` and `{first_layer_temperature[0]}` (PLA defaults **230 / 230 / 60 bed**) | Copied | Dual start still heats T0 and T1 when `is_extruder_used`. Left-only skips T1. Right-only heats **T1 only** then `T1` before `M190`/`G28` (`RightonlyExtruder.gcode`). |
| `G21` `G90` `M82` `M107` | `G21` `G90` `M83` `M107` | Changed | ideaMaker uses absolute E. PrusaSlicer wipe tower requires relative E, so the Dual profile emits `M83`. |
| `G28 X0 Y0` then `G28 Z0` | Same | Copied | Do not replace with `G28` or add `G29`. Left/dual home on `T0`. Right-only homes on `T1`. |
| `G1 Z15.0 F300` | Same | Copied | Clearance before purge. |
| `G92 E0` / `G1 F140 E29` / `G1 X20 Y0 F140 E30` / `G92 E0` | Same blob; wipe is `G1 X80 Y0 F140 E1` (relative). No extra `G1 X80 Y0 F9000` before `M1001`. | Changed XY | ideaMaker wipes to X20 then **stays at Z15**, travels to print-start XY, then drops Z. Stage 3: X20 is still under the fan if Z drops there, so the wipe is X80. Dual ideaMaker has no XY wipe (blob at home). |
| First approach after `M1001` | `G0 F9000 X… Y…` then `G0 F300 Z0.300` | Copied dialect | ideaMaker never drops Z at the purge. PrusaSlicer emits `G1 Z.3` then `G1 X… Y…`. `ensure_m99123_first.py` reorders (or splits combined XYZ) so XY happens at Z15, then Z. |
| `G1 F9000.0` / `M117 Printing...` / `M1001` | Same | Copied | Travel feed + start marker. |
| `SET_VELOCITY_LIMIT ACCEL=5000.00` and `SQUARE_CORNER_VELOCITY=10.00` | Same after `M1001`. Print profile: walls/infill/first **5000**, solid/top **2000**, travel **5000** from `Compensation Test.gcode` (ideaMaker 5.5.0.8810). `ensure_m99123_first.py` rewrites PrusaSlicer `M204 S` to `SET_VELOCITY_LIMIT ACCEL=….00` | Copied dialect + that cube’s per-feature accel | PrusaSlicer 2.9.6 Klipper flavor only emits `M204 S` ([GCodeWriter](https://github.com/prusa3d/PrusaSlicer/blob/version_2.9.6/src/libslic3r/GCode/GCodeWriter.cpp)); Klipper accepts `M204 S` or `SET_VELOCITY_LIMIT`. The cube stays at 5000 through walls/infill/first layer and drops to 2000 on later solid/top. `MulticolorRaise3d.gcode` was almost all 2000 print / 5000 travel because that job was solid-heavy. |
| `G29`, `M92`, `M218`, `M600`, `PRINT_START` macros | — | Omitted | Not in this file. Generic Klipper macros are not authoritative. |

## End G-code

ideaMaker emits a slicer-generated retract/Z-hop (`G1 F2400 E…`, `G0 F300 Z…`) then custom end. PrusaSlicer will emit its own retract before custom end G-code. Custom end starts at `M221`:

| ideaMaker (source) | PrusaSlicer equivalent | Action | Reason |
| --- | --- | --- | --- |
| `M221 T0 S100` / `G92 E0` / `M1002` | Same | Copied | Reset flow + end marker. |
| `M104 T0 S0` / `M221 T0 S100` / `M104 S0` / `M140 S0` / `M107` | Same pattern; tool index follows `is_extruder_used` | Copied | Left: T0 + bare `M104 S0`. Right: T1 + bare `M104 S0`. Dual: T0 and T1, **no** bare `M104 S0`. |
| `G91` / `G1 E-1 F300` / `G1 Z+0.5 E-5 X-20 Y-20 F9000.00` | Same | Copied | Relative retract and wipe. |
| `G28 X0 Y0` / `M84` / `G90` | Same | Copied | Home XY, disable steppers, restore absolute. |
| Raise Z to `max_layer_z+5` (izumi) | Omitted | Omitted | Not in this ideaMaker end. Relative `Z+0.5` is what the machine file does. |

## Dual printer (T0 + T1)

Source: `MulticolorRaise3d.gcode` for dual; `LeftonlyExtruder.gcode` / `RightonlyExtruder.gcode` for single-tool. XY offset is **not** sliced in (`extruder_offset = 0x0,0x0`); factory ~25 mm X lives in printer hardware. ideaMaker files only emit `;Extruder Offset #1: 0.000 0.000` (no `#2` comment).

| Dual behavior | ideaMaker (`MulticolorRaise3d.gcode`) | PrusaSlicer | Action |
| --- | --- | --- | --- |
| Heat / flow both tools | `M221`/`M104`/`M109` T0 and T1 at 230 °C, `M221 S94` | Same commands gated with `is_extruder_used`; PLA **230 °C** and M221 S94; slicer multiplier 1.00 | Copied |
| Home | Dual/left: `T0` then `G28`. Right-only: `T1` then `G28` | `{if is_extruder_used[0]}T0{else}T1{endif}` before `M190`/`G28` | Copied per file |
| Dual purge | `T1`: `G1 F200 E10` then `G1 F200 E-11.00`. `T0`: `G1 F200 E10` (no XY wipe). Then print-start XY at Z15, then Z | Same in-place prime. First approach after `M1001` is XY then Z (post-process). First later `T` to the retracted tool gets `G1 E11 F1500` (PrusaSlicer does not count start `E-11`) | Prime copied. Do not drop Z at home |
| Left-only purge | `LeftonlyExtruder.gcode`: blob + `G1 X20 Y0 F140 E30`, then print-start XY at Z15, then Z | Blob + `G1 X80 Y0 F140 E1`. First approach XY then Z | X80 from Stage 3 (fan hit the purge at X20 when Z dropped there) |
| Right-only purge | `RightonlyExtruder.gcode`: heat/select `T1`, same `F140 E29` + `G1 X20 Y0 F140 E30` as left (no `E10`/`E-11`, no `X40 Y0`) | Same as left at X80; last move `E1` under `M83` | Dual in-place `E10`/`E-11` is **not** used for T1-only |
| Unused tool after start | `M104 T1 S180` before layer 0 (T0 prints first) | Same when both tools used | Copied. Standby is **180 °C**, not `temperature-30` |
| Select first printing tool | Starts on T0 after dual purge | `{if initial_extruder == 0}T0{else}T1{endif}` | Copied |
| Preheat next tool | `M104 T{next} S230` inserted mid-print before the swap (gaps **64–2200** lines, median **762**, n=294 in this file) | `ensure_m99123_first.py` inserts `M104 T{n} S{M109}` ~30 s before each post-`M1001` swap `M109` using `M73` remaining minutes (fall back to ~400 lines if `M73` is missing; skip if the other tool’s stint is shorter than 50 lines). Swap still `M109`. | Copied as post-process. [Prusa placeholders](https://help.prusa3d.com/article/list-of-placeholders_205643): `next_extruder` is tool-change only, not before-layer. ideaMaker’s median gap is ~30 s. |
| Sequential objects | Not in these ideaMaker files | `complete_objects = 0`. Dual + wipe tower + sequential is a PrusaSlicer hard error — turn the tower off first. No between-object purge (swap is over object 2). Post-process fills skipped `;LAYER:N` on each object’s first layer and applies first-layer 90 % FLOW there. Check GUI clearance circles (height 80 / radius 90). | Documented. Wipe-tower min purge stays PrusaSlicer’s **15 mm³** default (not ideaMaker’s 20). |
| Tool-change | Travel `G0 F9000 X30 Y295`, `G92 E0`, `G1 F1200 E-11`, `M104 T{prev} S180`, `M109 T{next} S230`, `T`, then wipe-tower prime `E11` | Standby `S180` if previous is 0/1 (skip when `-1`); `M109` uses `first_layer_temperature` on layer 0 else `temperature`. Slicer emits `T` and the wipe tower. `retract_length_toolchange=11`. Post-process drops layer-2 `M104 S{print} T{idle}` so the parked nozzle stays 180 | Copied temps only. **Do not** copy park `X30 Y295` or the octagon. No `M218`, no Marlin `M116`/`P0` |
| Wipe tower | `;TYPE:WIPE-TOWER` octagon around ~X50 Y241 (X26–X75, Y217–Y266 in this file) | On. Position is a project setting in 2.9.6 (no `wipe_tower_x`/`wipe_tower_y` in the profile). Built-in default is around X180 Y140. Drag on the plater past T1 keep-out. Not ideaMaker’s XY | Copied the *feature* (need a tower for dual). Relative E (`M83`) so PrusaSlicer can emit it |
| T1 leftmost ~25 mm | Dual bounding box starts at **X25.384**. T1 print/tower min **X27.3**. T0 tower goes to **X23.9** | Slicer offset still `0x0,0x0`. Bed texture stripe + validator (T1 X < 25 after `M1001`). No `extruder_printable_area` (Orca/Bambu only; not in PrusaSlicer 2.9.6) | Copied firmware-offset policy. This dual file keeps T1 ≥ 27.3; 25 mm remains the factory-offset keep-out |
| First layer / skirt | First layer **0.300 mm**. Left/right files start on wall (no skirt). Dual file still has skirt at `F900` (15 mm/s), then 0.200 mm | **0.20mm Hyper Speed:** `first_layer_height = 0.3`, `skirts = 0`, first-layer walls and infill **50 mm/s**, accel **2000** | Copied first-layer height and 50 mm/s part feed. Dual skirt at 15 mm/s is not copied (start G-code already purges). PrusaSlicer cannot emit ideaMaker’s 50→75→100→125→150 ramp on layers 0–4. |
| Fan | Layer 1 `M106 S0`, layer 2 `S128` (50 %), layer 3+ `S255` | PLA: `disable_fan_first_layers = 1`, `full_fan_speed_layer = 3`, `min_fan_speed = max_fan_speed = 100` | Copied the 0 / 50 / 100 points. Other filaments keep operator XL cooling. |
| Print / travel speeds | Mix of 25–150 mm/s print (`F1500`…`F9000` with E); travel `F9000` (150 mm/s) | Single profile: walls **150**, infill/solid **120**, tops **100**, gap **100**, travel **150**; PLA volumetric **15 mm³/s** | Copied cruise numbers from `LeftonlyExtruder.gcode`. |
| End | Dual: `M221` T0 and T1 `S100` twice around `M1002`; `M104 T0/T1 S0`; no bare `M104 S0`. Right-only: `M221 T1` / `M104 T1 S0` / `M104 S0`. Then relative wipe; `G28 X0 Y0`; `M84` | Same, gated with `is_extruder_used` | Copied per file |
| Headers | Filament `#1` and `#2`; no Offset `#2` | Filament `#1`/`#2`; Offset `#1` only | Copied. Names are `[Raise3D] PLA` |

## Uncertain commands — required physical tests

| Uncertainty | Test |
| --- | --- |
| `M99123` payload copied from this file | Stage 2–3: note whether Hyper Speed checkmark appears; abort if the printer refuses the file. |
| `;Filament Name #1: [Raise3D] PLA` | Stage 2: confirm the touchscreen slot names still match. |
| T0-only preheat (no T1) | Stage 3: confirm left nozzle lifts/prints without forcing a T1 heat. |
| Left-only purge blob at homed origin then `X80 Y0` | Stage 3: confirm XY travel at Z15 then Z drop at the part; fan should not hit the blob. Purge stays on the plate, not on the clip. |
| Right-only: home on `T1`, same `X80 Y0` wipe with the right nozzle | Stage 6: confirm T1 lifts for home, wipe is on the plate, and first print stays right of ~X25. |
| Dual in-place purge `F200 E10` / `E-11` (no XY wipe) | Stage 6–7: confirm blobs form at home and do not hit the clip or a parked nozzle. |
| Per-feature `SET_VELOCITY_LIMIT` 5000 walls/infill vs 2000 solid/top | Stage 4: compare ringing to `Compensation Test.gcode`. Cadence will not match ideaMaker’s switch count exactly (PrusaSlicer emits `M204` per feature change, then post-process converts). |
| `M2000` pause (community, not in this file) | Stage 5 only; do not use on a long print first. |
| Absolute E (`M82`) under PrusaSlicer Klipper flavor | Stage 2: inspect G-code. Dual profile now uses `M83` / relative E for the wipe tower. |
| Firmware 25 mm X offset with slicer offset 0 | Stage 6: watch right nozzle path; abort if it is ~25 mm off the model or hits the left nozzle. |
| Tool-change standby 180 °C + PrusaSlicer wipe tower | Stage 7: confirm unused nozzle lifts, `M109` reaches temp, and the tower is where you placed it on the plater (not ideaMaker’s ~X50 Y241 octagon / park `X30 Y295`). |
| Next-tool `M104` ~30 s before swap `M109` | Stage 7: confirm the unused nozzle is heating during the other tool’s print; abort if ooze hits the part. Lead is from `M73` remaining time (~30 s); 400-line fallback if `M73` is missing. |

## Commands remaining Not implemented

- Pause/filament-runout recovery beyond documenting `M2000` as community
- ideaMaker `;Data start` / recover comment block
- Hyper Speed PLA material profile
- ideaMaker dual-file skirt at 15 mm/s (start G-code already purges; no skirt)
- ideaMaker 50→150 speed ramp on layers 0–4 (PrusaSlicer only has first-layer speed)
- `G29` mesh, `M92` steps, `M218` offsets
