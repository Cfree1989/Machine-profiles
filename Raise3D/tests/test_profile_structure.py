...
class VendorStructureTests(unittest.TestCase):
    ...
    def test_vendor_and_model_sections(self) -> None:
        ...
        self.assertEqual(self.ini["vendor"]["config_version"], "0.5.58")
        self.assertIn("0.5.58 ", IDX.read_text(encoding="utf-8"))
    ...
    def test_print_and_filament_are_tied_to_dual_printer(self) -> None:
        ...
        self.assertIn(';Filament Name:', self.ini["filament:*common*"]["start_filament_gcode"])
        quoted_starts = [
            line for line in self.raw.splitlines() if line.startswith("start_filament_gcode = ")
        ]
        self.assertGreaterEqual(len(quoted_starts), 2)
        self.assertTrue(all(line.startswith('start_filament_gcode = ";') for line in quoted_starts))
    ...
    def test_dual_printer_preset(self) -> None:
        ...
        self.assertEqual(p["retract_before_wipe"], "95%,95%")
        ...
        self.assertIn("first_layer_temperature[next_extruder]", p["toolchange_gcode"])
        self.assertIn("{if layer_num == 0}", p["toolchange_gcode"])
    ...
    def test_print_matches_ideamaker_hyper_speed(self) -> None:
        ...
        self.assertEqual(p["default_acceleration"], "5000")
        ...
        self.assertEqual(p["perimeter_acceleration"], "5000")
        self.assertEqual(p["external_perimeter_acceleration"], "5000")
        self.assertEqual(p["infill_acceleration"], "5000")
        self.assertEqual(p["solid_infill_acceleration"], "2000")
        self.assertEqual(p["top_solid_infill_acceleration"], "2000")
        self.assertEqual(p["elefant_foot_compensation"], "0.20")
        ...
        self.assertNotIn("wipe_tower_y", p)
        self.assertEqual(p["complete_objects"], "0")
        self.assertNotIn("filament_minimal_purge_on_wipe_tower", self.raw)
    ...
    def test_wizard_filaments_are_raise3d_named_and_xl_temps(self) -> None:
        ...
        self.assertEqual(pla["min_fan_speed"], "100")
        self.assertEqual(pla["max_fan_speed"], "100")
        self.assertEqual(pla["disable_fan_first_layers"], "1")
        self.assertNotIn("X50 Y140", self.raw)
        self.assertEqual(pla["min_print_speed"], "100")
    ...
    def test_print_post_process_runs_hyper_speed_scripts(self) -> None:
        ...
        self.assertIn("C:\\\\Repos\\\\Machine-profiles\\\\Raise3D\\\\scripts\\\\", post)
