...
Print is a single **0.20mm Hyper Speed** preset measured from this machine’s ideaMaker jobs: 0.20 mm layers, 0.30 mm first layer at 50 mm/s, 0.40 mm lines (0.48 first), walls 150, infill/solid 120, tops 100, travel 150, accel 5000 walls/infill/first and 2000 solid/top (`Compensation Test.gcode`). PLA is **230 °C**, M221 S94, slicer multiplier 1.00, first-layer FLOW 90% and top FLOW 106% in post-process, fan 0 / 50% / 100%, cooling floor 100 mm/s. Other filaments keep the operator `* XL` temps, cooling, and PETG/TPU retract; volumetric is `min(XL, 15)` (TPU 2.5, PA-CF 8). Dual unused-nozzle standby stays **180 °C**. Filament presets are compatible only with this Dual 0.4 printer, and the printer notes carry `NO_TEMPLATES`, so PrusaSlicer's own Template filaments (Generic PLA, Overture PLA, Prusa ABS @Template) are hidden for it: the filament list is the six Raise3D presets only.
...
1. Copy `vendor/Raise3D.ini`, `vendor/Raise3D.idx`, and the whole `vendor/Raise3D/` folder to `%APPDATA%\PrusaSlicer\vendor\`. That folder holds the plater and wizard assets: `PRO2PLUS_HS_DUAL_bed.stl` (13.375 × 13 in plate plus front handle, centred on the 305 × 305 printable area), `PRO2PLUS_HS_DUAL_texture.svg` (BuildTak surface, faint 10 / 50 mm grid, hatched orange T1 keep-out band), and `PRO2PLUS_HS_DUAL_thumbnail.png` (180 × 256 Configuration Wizard photo). PrusaSlicer 2.9.6 only draws the vendor plate when **both** the STL and the SVG are present; with the SVG alone (bundles before 0.5.49) it silently fell back to the plain grid.
...
Import Config Bundle does not install the `vendor/Raise3D/` assets (bed STL, texture SVG, thumbnail). Copy that folder as in A if you want the plate and the hatched orange T1 keep-out band on the plater. Wipe tower position lives on the project, not this profile; drag it on the plater.
...
4. Slice a small test. Wipe tower is on. PrusaSlicer 2.9 places it on the project (built-in default is around X180 Y140, not a profile setting). Drag it so T1 can reach it and it stays off the hatched keep-out. Unused nozzle drops to 180 °C on tool change and stays there until its next swap (it is not reheated on layer 2). The hatched orange band on the bed is T1 keep-out (leftmost ~25 mm); it is hatched rather than labelled because PrusaSlicer's SVG renderer (nanosvg) ignores text.
...
Port review (`docs/PORT_REVIEW.md`): 0.5.51 findings; 1.1–1.6 applied in 0.5.52–0.5.54; sequential layers/FLOW and time-based preheat in 0.5.55. 0.5.57 keeps PrusaSlicer native small-perimeter size, retract speeds, and solid accel (ideaMaker-only rewrites dropped). Wipe-tower purge stays PrusaSlicer's 15 mm³ default.
...
- PLA at **230 °C** / M221 S94 / slicer multiplier 1.00 / 60 °C bed, matching ideaMaker Standard PLA. Post-process applies first-layer FLOW 90% and top FLOW 106%. PETG/TPU/ASA/PA-CF/ABS-GF temps, cooling, and PETG/TPU retract still match the operator `* XL` presets; volumetric is min(XL, Hyper FFF L1 15 mm³/s). Dual standby stays 180 °C from Multicolor (filament idle is not used in Raise3D tool-change G-code). If a previous Ellis print left Klipper PA at 0.068, restart the printer (or set firmware PA) before judging this profile — we do not emit `SET_PRESSURE_ADVANCE`.
- `SET_VELOCITY_LIMIT` walls/infill/first 5000, solid/top 2000, travel 5000 after converting PrusaSlicer `M204 S` (`Compensation Test.gcode`). Cadence will not match ideaMaker’s switch count exactly.
- Print speeds follow the ideaMaker job: walls 150, infill/solid 120, tops 100, first layer 50. Small walls use PrusaSlicer’s Ø13 threshold at 75 mm/s. PrusaSlicer cannot emit the 50→75→100→125→150 ramp on layers 0–4. Bridge 30 mm/s at 90% flow is not in those files (no `;TYPE:BRIDGE`).
- `M2000` pause (community; not in the ideaMaker file).
- Dual: electronic lift on `T0`/`T1`, firmware XY offset (~25 mm X; slicer offset 0), in-place dual prime (`F200 E10` / `E-11`) when both tools are used. After purge, first print travel is XY at Z15 then Z (ideaMaker; PrusaSlicer would drop Z at the purge). Tool-change standby 180 °C. The first swap to the other tool unretracts 11 mm in post-process (start `E-11` is invisible to PrusaSlicer). Right-only uses the same `X80 Y0` wipe as left, after homing on T1. Wipe tower is on (relative E); place it on the plater (PrusaSlicer 2.9 default is around X180 Y140). This ideaMaker dual file placed the octagon around ~X50 Y241. Next-tool `M104` is inserted ~30 s before swap `M109` from `M73` remaining time (400-line fallback if `M73` is missing; ideaMaker gaps 64–2200, median 762). Wipe-tower min purge stays PrusaSlicer's 15 mm³ (not ideaMaker's 20). T1 in this dual file stays ≥ ~X27; validator keep-out is X < 25.
...
- Sequential printing: Complete individual objects is still off by default. Dual sequential **cannot** keep the wipe tower — turn the tower off first or PrusaSlicer errors. There is no purge between objects; the nozzle swap happens over object 2. Post-process fills the skipped `;LAYER:N` on each object's first layer and applies first-layer 90 % FLOW there. Check the clearance circles in the GUI before you rely on them: `extruder_clearance_height` 80 mm (Copperhead), `extruder_clearance_radius` 90 mm so the whole head is inside the cylinder (T0 front-right ~86). Operator box: T0 L42.5/R70/F50/B30, T1 L67.5/R45/F50/B30. ideaMaker stock was gantry 65 and T0 37/63/45/70, T1 62/38/45/70.
...
| 7 | Dual color: 180 °C standby, lift, alignment (firmware offset), wipe tower (drag it on the plater; keep it off the leftmost ~25 mm). Abort if nozzles collide, T1 is shifted ~25 mm, or purge lands on the part. |
...
1. Slice a small part (assign the tools you will use). Print profiles run `scripts\ensure_m99123_first.py` then `scripts\validate_gcode.py` as post-processing ([PrusaSlicer post-processing](https://help.prusa3d.com/article/post-processing-scripts_283913)): that moves `M99123` to line 1, converts `M204` to `SET_VELOCITY_LIMIT`, inserts next-tool preheat (~30 s from `M73`, else ~400 lines), reorders the first print approach to XY at purge height then Z (ideaMaker), writes RaiseTouch `;PRINTING_TIME:` / `;REMAINING_TIME:` immediately before each `;LAYER:N` (ideaMaker Tune clock; synthesizes `;LAYER:0` after `M1001` because PrusaSlicer skips Before layer change G-code on the first layer, and fills sequential object-first layers that skipped the same G-code) plus `;Print Time:` from PrusaSlicer `M73` and estimated-time comments, copies the slicer's `;HEIGHT:` to after `;LAYER:N` / `;Z:` (do **not** put `HEIGHT` in Before layer change G-code — PrusaSlicer reserved keyword), writes `;Bounding Box:` as ideaMaker `xmin xmax ymin ymax zmin zmax` from print moves, and for PLA (`M221 S94`) scales first-layer print E by 90% (every object's first layer) and top-solid E by 106%. Then `validate_gcode.py` rejects unsafe G-code (a validator error aborts export). G-code thumbnails are off so a PNG block is not sitting in front of the header. On the printer, name the loaded slots `[Raise3D] PLA` (or `[Raise3D]` plus the material type) so Filament Status can go green. Hyper Speed may stay yellow: copying `;Sliced by ideaMaker` does not unlock that check.
...
3. Read the first ~80 lines and the end sequence. Confirm `M99123` is line 1, `G28 X0 Y0` then `G28 Z0`, `M1001` / `M1002`, no `G29` / `M92` / `M218`. Left-only: heat T0, home T0, purge `X80 Y0`. Right-only: heat T1, home T1, same `X80 Y0` wipe. Dual: home T0, in-place `F200 E10` / `E-11`. After purge, first print move is XY at Z15 then Z (ideaMaker). ideaMaker references still wipe to `X20`; the profile uses X80 so the fan clears the blob if Z were dropped there. Single-tool end turns off only that tool plus bare `M104 S0`. Tool-change standby `S180`; wipe tower is where you placed it on the plater (PrusaSlicer 2.9 default is around X180 Y140; this ideaMaker dual file used ~X50 Y241). Validator rejects T1 print moves with X < 25 mm.
